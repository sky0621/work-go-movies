...
// [MEMO]Loggerのテストはひとまず保留

func TestOpenStrage(t *testing.T) {
	t.Log("対象ファイルがあれば開けるはず")
	s := StorageJSON{JSONPath: "testdata/storagejson/OpenStorage/movies-persistence.json"}
	err := s.OpenStorage()
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenStrage_NoFile(t *testing.T) {
	...
	s := StorageJSON{JSONPath: "testdata/storagejson/OpenStorage/movies-persistence2.json"}
	err := s.OpenStorage()
	if err != nil {
		t.Fatal(err)
	}
