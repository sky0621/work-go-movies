...
func parseFlag() (*c.Arg, error) {
	...
	var logDir string
	var isDebug bool
	flag.StringVar(&grpc2sPort, "grpc2s", ":7110", "GRPC(対 サーバ)接続先ポート")
	flag.StringVar(&grpc2sPort, "gs", ":7110", "GRPC(対 サーバ)接続先ポート")
	...
	arg, err := c.NewArg(grpc2sPort, addr, logDir, isDebug)
	if err != nil {
		return nil, err
	}
