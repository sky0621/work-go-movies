...
func TestSetupLog(t *testing.T) {
	t.Log("指定のパスに「movies-server.log」が存在する場合、ロガーの設定が正常終了するはず")
...
func TestSetupLog_FileNotExists(t *testing.T) {
	t.Log("指定のパスに「movies-server.log」が存在しない場合、新規に作成してロガーの設定が正常終了するはず")
...
func TestDebug(t *testing.T) {
	...
	defer os.Remove("testdata/logging/debug/movies-server.log")
	defer logfile.Close()
	logr := logger{isDebugEnable: true}
	logr.debug("FunctionA", "test")
	b, err := ioutil.ReadFile("testdata/logging/debug/movies-server.log")
	if err != nil {
		t.Fatal(err)
	}
	s := string(b)
...
func TestInfo(t *testing.T) {
	...
	defer os.Remove("testdata/logging/info/movies-server.log")
	defer logfile.Close()
	logr := logger{isDebugEnable: false}
	logr.info("FunctionA", "test")
	b, err := ioutil.ReadFile("testdata/logging/info/movies-server.log")
	if err != nil {
		t.Fatal(err)
	}
	s := string(b)
...
func TestError(t *testing.T) {
	...
	defer os.Remove("testdata/logging/error/movies-server.log")
	defer logfile.Close()
	logr := logger{isDebugEnable: false}
	logr.error("FunctionA", "test")
	b, err := ioutil.ReadFile("testdata/logging/error/movies-server.log")
	if err != nil {
		t.Fatal(err)
	}
	s := string(b)
